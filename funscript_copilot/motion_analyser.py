    ...
    def start(self):
        ...
        while self.video.isOpened() and not self.stop:
            ...
            if len(y_batch) >= self.batch_size:
                ...
                batch_prediction = np.transpose(np.array(self.ipca.transform(y_batch)))
                y_batch = []
                for i in range(self.n_components):
                    prediction_pca[i].extend(batch_prediction[i])

        if self.n_components == 2:
            result = np.array(prediction_pca[0]) + np.array(prediction_pca[1])
            plt.plot(result)
        else:
            for i in range(self.n_components):
